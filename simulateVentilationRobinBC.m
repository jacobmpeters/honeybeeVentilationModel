...
[plotMode,T_amb,L_x,saveInterval,Dv,Dt,m,c,nSteps,alpha,seed] = ...
...
history(1).rho = horzcat(0,repmat(50,1,nBins-2),0); 
...
%     colorbar
    ...
    ylabel('$\hat{x}$','interpreter','latex','FontSize',18)
    xlabel('$\hat{t}$','interpreter','latex','FontSize',18)
    title('$\Theta = T/T_{h}$','interpreter','latex','FontSize',20);
    colorbar('peer',ax(3))
...
function [plotMode,T_amb,L_x,saveInterval,Dv,Dt,m,c,nSteps,alpha,seed] =...
    extractOptionalInputs(optionalInput)
